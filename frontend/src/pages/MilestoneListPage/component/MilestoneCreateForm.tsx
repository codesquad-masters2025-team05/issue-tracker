...
export function MilestoneCreateForm({ onClose }: MilestoneCreateFormProps) {
  ...
  const isValidDateFormat = (value: string) =>
    value === '' || /^\d{4}\.\s?\d{2}\.\s?\d{2}$/.test(value);
  ...
  const onCreateMilestone = () => {
    ...
    milestoneCreateMutate({
      payload: {
        name,
        description,
        // 날짜에서 공백제거 후 전달 (ex: 2025.05.31)
        deadline: deadline ? deadline.replace(/\s/g, '') : undefined,
      },
    });
  };
  ...
  return (
    <div className="w-[1280px] flex flex-col gap-8 p-8 bg-[var(--neutral-surface-strong)] border border-[var(--neutral-border-default)] rounded-2xl">
      ...
      <div className="flex flex-row gap-4 justify-end">
        ...
        <CompleteButton onClick={onCreateMilestone} disabled={!name || !!deadlineError} />
      </div>
    </div>
  );
}
...
function MilestonePreview({ name, description, deadline }: MilestonePreviewProps) {
  return (
    <div className="w-[288px] h-[153px] flex flex-col justify-center gap-2 px-6 border border-[var(--neutral-border-default)] rounded-2xl bg-white">
      <span className="font-display-bold-16 text-[var(--neutral-text-strong)] break-words">{name || '마일스톤 이름'}</span>
...
function DeadlineInput({ value, onChange, error }: DeadlineInputProps) {
  return (
    <div className="flex flex-col gap-1">
      <div className="flex items-center h-10 rounded-[16px] px-4 w-[240px]
      ...
      {error && (
        <span className="text-[var(--danger-text-default)] font-display-medium-12">{error}</span>
      )}
    </div>
  );
}
